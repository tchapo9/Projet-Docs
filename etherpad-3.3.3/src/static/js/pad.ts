...
const getParameters = [
  ...
  },
  {
    ...
    checkVal: null,
    callback: (val) => {
      clientVars.initialOptions = clientVars.initialOptions || {};
...
const pad = {
  ...
  handleClientMessage: (msg) => {
    ...
    } else if (msg.type === 'LUMEN_SET_READONLY') {
      if (msg.readonly) {
        // Lumen Docs — révocation temps réel du droit de saisie.
        window.clientVars.readonly = true;
        $('body').addClass('readonly').removeClass('readwrite');
        chat.hide();
        $('#myusernameedit').attr('disabled', true);
        $('#chatinput').attr('disabled', true);
        $('#chaticon').hide();
        ...
        window.clientVars.readonly = false;
        // Si l'URL contient encore readOnly=true, recharger sans ce param
        // pour que le serveur rende la toolbar complète.
        const url = new URL(window.location.href);
        if (url.searchParams.has('readOnly')) {
          url.searchParams.delete('readOnly');
          window.location.href = url.toString();
        } else {
          $('body').removeClass('readonly').addClass('readwrite');
          chat.show();
          $('#myusernameedit').attr('disabled', false);
          $('#chatinput').attr('disabled', false);
          $('#chaticon').show();
          $('#options-chatandusers').parent().show();
          $('#options-stickychat').parent().show();
          padeditor.ace.callWithAce((ace) => {
