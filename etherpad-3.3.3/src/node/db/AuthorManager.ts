...
exports.getAuthorId = async (token: string, user: object) => {
  ...
  if (!authorId) {
    // Lumen Docs — aligner l'authorID d'une session authentifiée par token sur
    // celui que l'intégrateur pré-attribue via createAuthorIfNotExistsFor(token).
    // Cette API écrit le mapping `mapper2author:<token>` ; en le consultant ici,
    // la session utilise le MÊME authorID que celui retourné à lumens, ce qui
    // permet de cibler précisément cette session lors d'une révocation.
    const mapped = await db.get(`mapper2author:${token}`);
    if (mapped != null) authorId = mapped;
  }
